...
const server = http.createServer((req, res) => {
  if (req.method === "GET" && req.url === "/") {
    ...
    let signupForm = fs.readFileSync("./signUp.html", "utf-8"); // appending html file
    res.setHeader("Content-Type", "text/html");
    res.end(signupForm);
    ...
    req.on("end", () => {
      ...
      if (username && password) {
        ...
        fs.appendFileSync("./user.txt", userEntry, "utf-8"); // appending data in user.txt file
    ...
    try {
      ...
      // res.end(allUsers); // read both name and password
      const usernames = allUsers //usernames Extract names only
        .split("\n")
